  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      children: <Widget>[
        ...
        const Padding(
          padding: EdgeInsetsDirectional.only(top: 8.0, start: 8.0),
          child: Text('unconstrained layout')
        ),
        ConstrainedBox(
          ...
          child: CarouselView(
            itemExtent: 330,
            shrinkExtent: 200,
            children: CardInfo.values.map((CardInfo cardInfo) {
              return UncontainedLayoutCard(cardInfo: cardInfo);
            }).toList(),
          )
        )
      ]
...
class UncontainedLayoutCard extends StatelessWidget {
  const UncontainedLayoutCard({super.key, required this.cardInfo});
  ...
  Widget build(BuildContext context) {
    return ColoredBox(
      color: Colors.primaries[cardInfo.index % Colors.primaries.length].withOpacity(0.5),
      child: Column(
